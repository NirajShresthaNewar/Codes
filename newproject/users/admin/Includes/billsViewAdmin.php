...
<form method="GET" id="filterForm">
   <!-- <label for="user_filter">Filter by User ID or Email:</label>-->
    <input type="text" id="user_filter" name="user_filter" placeholder="Id or Email" value="<?php echo isset($_GET['user_filter']) ? $_GET['user_filter'] : ''; ?>">
...
if ($result->num_rows > 0) {
    ...
    <button type="button" onclick="submitForm('dataForm<?= $row['bill_id'] ?>')">Update</button>

                      
            <!-- Delete button to delete page -->
            <button type="button" onclick="deleteBill(<?= $row['reading_id'] ?>)">Delete</button>
        </td>
                </tr>
            <?php endwhile; ?>
        </tbody>
